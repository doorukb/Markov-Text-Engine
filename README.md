...
## Order and corpus size

Higher order means more context and more coherent output, but it also means the chain is more likely to reproduce the source text verbatim. For small corpora the crossover happens quickly. Order 1 is generative. Order 3 on a short corpus is essentially a lookup table.

The vocabulary size directly determines matrix dimensions. For order-1, the matrix is `vocab_size x vocab_size`. Keep corpora small enough that this stays tractable, or the heatmap visualization becomes unreadable regardless.
...
```bash
# You want an output of order 2 with maximum size of 150 tokens
